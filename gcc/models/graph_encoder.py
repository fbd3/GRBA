...
class GraphEncoder(nn.Module):
    ...
    def __init__(
        ...
        if gnn_model == "mpnn": #========= Remove GAT MPNN =======
            pass
        elif gnn_model == "gin":
        ...
        # self.node_freq_embedding = nn.Embedding(  num_embeddings=max_node_freq + 1, embedding_dim=freq_embedding_size )
        if degree_input:
            self.degree_embedding = nn.Embedding(  num_embeddings=max_degree + 1, embedding_dim=degree_embedding_size )

        # self.edge_freq_embedding = nn.Embedding( num_embeddings=max_edge_freq + 1, embedding_dim=freq_embedding_size )
