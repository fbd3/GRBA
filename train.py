...
#* OK opt_model_name_and_load_path_and_tb_folder
                            ...
                            opt.dataset,
                                opt.model, opt.num_layer,
                                        opt.learning_rate, opt.weight_decay,
                                                            opt.batch_size,
                                                                    opt.hidden_size, opt.num_samples,
                                                                                            opt.nce_t,opt.nce_k,opt.rw_hops,opt.restart_prob,
                                                                                                                                        opt.aug,opt.finetune,
                                                                                                                                                opt.degree_embedding_size,
                                                                                                                                                opt.positional_embedding_size,opt.alpha,
    )
...
#* OK copy weights from model to model_ema When pre-training MOCO
...
def main(args):
    ...
    ############################################################
    #X print("Use GPU: {} for training".format(args.gpu))
    #X assert args.positional_embedding_size % 2 == 0 # 必须是偶数
    #X print("setting random seeds")
    ...
    if args.finetune: # GraphClassificationDatasetLabeled 带标签
        if args.dataset in GRAPH_CLASSIFICATION_DSETS:
    ...
    #* When pre-training at first
        ...
        print("++++++++++++++++++++++++++++++++++++++++++++++OKKKKKKKKK++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
        ##########################################
        #X dgl.data.utils.load_labels("./data/small.bin")
        ##########################################
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if args.finetune:
            ...
            loss = train_moco( epoch,  train_loader,  model,  model_ema, contrast, 
                criterion, optimizer,  sw,   args, )
        time2 = time.time()
        ...
        if epoch % args.save_freq == 0:
            ...
            del state
        # region saving the model ----------------每一轮都有 current
        # print("==> Saving...")
        # state = {
        #     "opt": args,
        #     "model": model.state_dict(),
...
def bugs():
    ...
    # #########
    # ......
    # tum_0.999', tb_freq=250, tb_path='tensorboard', weight_decay=1e-05)
    # Use GPU: 0 for training
    # setting random seeds
    # before construct dataset 8.948265075683594
    # Traceback (most recent call last):
    #   File "train.py", line 827, in <module>
    #     main(args)
    #   File "train.py", line 560, in main
    #     num_copies=args.num_copies,
    #   File "/Users/gsq/Desktop/GCC-master/gcc/datasets/graph_dataset.py", line 58, in __init__
    #     graph_sizes = dgl.data.utils.load_labels(dgl_graphs_file)[
    #   File "/Applications/anaconda3/envs/sslpy37/lib/python3.7/site-packages/dgl/data/graph_serialize.py", line 172, in load_labels
    ...
    # dgl._ffi.base.DGLError: [02:52:23] /Users/xiangsx/work/dgl/dgl/src/graph/graph_serialize.cc:202: Check failed: magicNum == kDGLSerializeMagic (5789751444030890300 vs. 15937764024707752255) : Invalid DGL files
    # Stack trace:
    #   [bt] (0) 1   libdgl.dylib                        0x000000011ad7cfa9 dmlc::LogMessageFatal::~LogMessageFatal() + 57
    #   [bt] (1) 2   libdgl.dylib                        0x000000011b5bd4d2 dgl::serialize::LoadDGLGraphs(std::__1::basic_string<char, std::__1::char_traits<char>, std::__1::allocator<char> > const&, std::__1::vector<unsigned long long, std::__1::allocator<unsigned long long> >, bool) + 466
    #   [bt] (2) 3   libdgl.dylib                        0x000000011b5c5d6a std::__1::__function::__func<dgl::serialize::$_2, std::__1::allocator<dgl::serialize::$_2>, void (dgl::runtime::DGLArgs, dgl::runtime::DGLRetValue*)>::operator()(dgl::runtime::DGLArgs&&, dgl::runtime::DGLRetValue*&&) + 746
    #   [bt] (3) 4   libdgl.dylib                        0x000000011b566f26 DGLFuncCall + 70
    #   [bt] (4) 5   core.cpython-37m-darwin.so          0x0000000109fc149c __pyx_f_3dgl_4_ffi_4_cy3_4core_FuncCall(void*, _object*, DGLValue*, int*) + 460
    #   [bt] (5) 6   core.cpython-37m-darwin.so          0x0000000109fc5a27 __pyx_pw_3dgl_4_ffi_4_cy3_4core_12FunctionBase_5__call__(_object*, _object*, _object*) + 55
    #   [bt] (6) 7   python3.7                           0x000000010686a5a3 call_function + 1379
    #   [bt] (7) 8   python3.7                           0x0000000106852f39 _PyEval_EvalFrameDefault + 30809
    #   [bt] (8) 9   python3.7                           0x000000010686a26b call_function + 555